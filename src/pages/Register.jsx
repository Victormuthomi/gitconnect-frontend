...
function Post({ post, onReact }) {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`http://172.18.0.3:8080/api/posts/${post.id}/comments`);
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      // Fetch username if not included in response
      if (!newComment.username) {
        try {
          const userResponse = await axios.get(`http://172.18.0.3:8080/api/users/${newComment.user_id}`);
          newComment.username = userResponse.data.username || "Unknown";
        } catch (error) {
        ...
        {comments.length > 0 && (
          <div className="mt-2">
            {comments
              ...
              .map((cmt, index) => (
                <p key={index} className="text-sm text-gray-700 dark:text-gray-300 mt-1">
